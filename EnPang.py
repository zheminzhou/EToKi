...
def get_uclust(prefix, gene, priority) :
    ...
    subprocess.Popen('{0} -sortedby other -cluster_smallmem {1}.gene.sorted -uc {1}.gene.uc -centroids {1}.gene.reference -id {2} -query_cov {3} -target_cov {3}'.format(params['ublast'], prefix, params['clust_difference'], params['clust_match_prop']).split()).wait()
