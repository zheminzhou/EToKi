...
class recHMM(object) :
    ...
    def predict(self, mutations, branches, sequences, missing, marginal, prefix='RecHMM') :
    ...
    def margin_predict(self, marginal=0.9) :
        ...
        for name, dm, dr, observation, stat in zip(self.branches, self.model['posterior']['theta'], self.model['posterior']['R'], self.observations, status) :
            ...
            res[name] = dict(#stat=dict(zip(tuple(zip(obs.T[3], obs.T[4])), stat['gamma'].T[0])),
                             sketches=[p[:4]+p[6:] for p in path if p[2]-p[1] > 0 and p[5] >= marginal],
...
def RecHMM(args) :
    ...
    if not args.report :
        model.predict(mutations, branches=branches, sequences=sequences, missing=missing, marginal=args.marginal, prefix=args.prefix)
