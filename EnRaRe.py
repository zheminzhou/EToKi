...
class RecHMM(object) :
    ...
    def initiate(self, observations, init) :
        ...
        for cutoff in cutoffs :
            ...
            if rec.shape[0] :
                ...
                model['delta'], model['v'], model['v2'] = max(0.00002, min(0.005, (rec[0]+rec2[0])/(rec2[1] + rec[1]))), np.min([0.5, (rec[2]+rec2[2])/(rec2[1] + rec[1])]), np.min([0.5, (rec2[2]+1)/(rec2[1]+1)])
    ...
    def estimation(self, model, branch_measures) :
        ...
        if prediction['v'] < max_br/2.:
            prediction['v'] = max( max_br/2., model['v'] )
